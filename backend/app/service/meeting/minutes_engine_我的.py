...
class Config:
    ...
    MODEL_NAME = "deepseek-v4-pro" #"qwen-long"
    MODEL_TEMPERATURE = 0.2
    MODEL_MAX_TOKENS = 8192 # 4000
...
def save_result(result, rag_docs=None, output_dir="./output"):
    ...
    md_path = os.path.join(output_dir, f"meeting_minutes_{ts}.md")
    ...
    if rag_docs is not None:
        rag_json_path = os.path.join(output_dir, f"meeting_rag_{ts}.json")
        rag_output = {
            "version": "1.0",
    ...
    # 3. 保存元信息（调试/归档用）
    meta_path = os.path.join(output_dir, f"meeting_meta_{ts}.json")
...
def generate_minutes(text: str, title: str, output_dir: str,
    ...
    if not metadata.get("title") or metadata["title"] == "会议":
        metadata["title"] = title or "会议座谈纪要"  # 给个默认标题
